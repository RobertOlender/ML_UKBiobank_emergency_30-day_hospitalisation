...
#convert all 1/0s to Y/N's. 
train <- mutate_at(train, vars(3:35), funs(ifelse(. == 1, "Y", "N")))
test <- mutate_at(test, vars(3:35), funs(ifelse(. == 1, "Y", "N")))

#"One hot encoding" independent features in train dataset. 

lab_train <- train[,8]
dummy <- dummyVars(" ~ .", data = train[,-8])
newdata <- data.frame(predict(dummy, newdata = train[,-8]))
train <- cbind(newdata, lab_train)

#"One hot encoding" independent features in test dataset. 

lab_test <- test[,8]
dummy <- dummyVars(" ~ .", data = test[,-8])
newdata <- data.frame(predict(dummy, newdata = test[,-8]))
test <- cbind(newdata, lab_test)

train$lab_train <- as.factor(train$lab_train)
...
grid_tune <- expand.grid(
  ...
  max_depth = 4, #c(2,3,4), 
  eta = 0.3, #c(0.05, 0.1, 0.3), 
  gamma = 0.1, #c(0, 0.1, 0.5, 1.0),  
  colsample_bytree = 1, #c(0.5, 1.0),   
  min_child_weight = 1, #c(1, 1.5, 2), 
